...
OssmDiscovery::OssmDiscovery() : scanCallbacks_(*this) { devices_.reserve(kMaxDevices); }
...
bool OssmDiscovery::startScan(uint32_t durationMs) {
    if (!scan_ || !eventQueue_) return false;
...
size_t OssmDiscovery::deviceCount() const { return devices_.size(); }
...
bool OssmDiscovery::isScanning() const { return scan_ && scan_->isScanning(); }

void OssmDiscovery::handleAdvertisedDevice(const NimBLEAdvertisedDevice* device) {
    if (!eventQueue_ || !device) return;
    if (!isOssmDevice(device)) return;
