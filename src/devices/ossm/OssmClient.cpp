...
bool OssmClient::begin() {
    ...
    const BaseType_t taskCreated = xTaskCreate(
        workerEntry, "ossm-worker", kWorkerStackSize, worker_, kWorkerPriority, &workerTask_);
...
bool OssmClient::connect(const NimBLEAddress& address) {
    if (!initialized_ || address.isNull())
        return false;
...
OssmClient::ModeState OssmClient::modeState() const {
    return modeState_.load();
}
...
bool OssmClient::setSpeed(int speed) {
    ...
    if (requestedSpeed > 0) {
        ...
        if (confirmedEpoch != epoch || connectionState_.load() != ConnectionState::Connected ||
            modeState_.load() != ModeState::Ready || !ready_.load()) {
            return false;
        }
